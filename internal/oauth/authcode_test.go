...
func TestExchangeAuthCode(t *testing.T) {
	...
	tr, err := ExchangeAuthCode(srv.Client(), provider, "authcode123", "verifier123", "http://localhost:1/")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExchangeAuthCodeOmitsClientSecretForPublicClients(t *testing.T) {
	...
	if _, err := ExchangeAuthCode(srv.Client(), provider, "code", "verifier", "http://localhost/"); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestExchangeAuthCodeError(t *testing.T) {
	...
	_, err := ExchangeAuthCode(srv.Client(), provider, "code", "verifier", "http://localhost/")
	if err == nil {
...
func TestServeOnceReturnsCode(t *testing.T) {
	...
	go func() {
		code, err := ServeOnce(context.Background(), ln)
		resultCh <- struct {
			code string
			err  error
		}{code, err}
