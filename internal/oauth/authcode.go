...
func ExchangeAuthCode(client *http.Client, provider config.ProviderConfig, code, verifier, redirectURI string) (TokenResponse, error) {
	params := url.Values{}
...
// LocalPort returns the port ListenLocal bound to.
...
const redirectFailureHTML = `<html><head><title>Authorization result</title></head>` +
	`<body><p>Authorization failed: %s. You may close this window and check the terminal.</p></body></html>`
...
func ServeOnce(ctx context.Context, ln net.Listener) (string, error) {
	...
	mux.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		...
		if res.errCode != "" {
			...
			fmt.Fprintf(w, redirectFailureHTML, msg)
		} else {
