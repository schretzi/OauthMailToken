...
func postForm(client *http.Client, endpoint string, params url.Values) (TokenResponse, int, error) {
	...
	if err := json.Unmarshal(body, &tr); err != nil {
		return TokenResponse{}, resp.StatusCode, fmt.Errorf("decoding response from %s: %w (body=%q)", endpoint, err, body)
	}

	if tr.Error != "" {
		return tr, resp.StatusCode, &APIError{Code: tr.Error, Description: tr.ErrorDescription}
	}
	if resp.StatusCode >= 400 {
		return tr, resp.StatusCode, fmt.Errorf("%s returned HTTP %d: %s", endpoint, resp.StatusCode, body)
	}
	if tr.AccessToken == "" {
		...
		return tr, resp.StatusCode, fmt.Errorf("%s returned no access_token (and no error) - response: %s", endpoint, body)
	}
