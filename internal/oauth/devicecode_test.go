...
func TestRequestDeviceCode(t *testing.T) {
	...
	dc, err := RequestDeviceCode(srv.Client(), provider)
	if err != nil {
...
func TestPollDeviceTokenPendingThenSuccess(t *testing.T) {
	...
	tr, err := PollDeviceToken(srv.Client(), provider, dc, sleep, onTick)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestPollDeviceTokenDeclined(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]any{"error": "authorization_declined"})
	}))
	defer srv.Close()

	provider := config.ProviderConfig{TokenEndpoint: srv.URL}
	_, err := PollDeviceToken(srv.Client(), provider, DeviceCodeResponse{}, func(time.Duration) {}, nil)
...
func TestPollDeviceTokenOtherError(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		json.NewEncoder(w).Encode(map[string]any{"error": "invalid_client", "error_description": "bad secret"})
	}))
	defer srv.Close()
