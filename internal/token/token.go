...
func Store(account string, t *Token) error {
	// #nosec G117 -- serializing the access/refresh tokens is exactly what
	// this function is for. The result goes to the OS keyring (see
	// keyring.Set below), never to a log, a file or the network.
...
func Load(account string) (*Token, error) {
	...
	if count, chunked := strings.CutPrefix(data, chunkedMarkerPrefix); chunked {
